        ...
        if choice == "1":
            ...
            while player_1.total_score <= 100 and anna.total_score <= 100:
                if player_1.is_holding is False:
                    ...
                    choice = input(f"{player_1.name} - write roll(r) to continue"
                                   + " and hold(h) to save score: ")
            ...
            if player_1.total_score >= 100:
                ...
                score_board.collect_score(player_1.name, player_1.total_score,
                                player_1.longest_streak, "highscore.txt")
            ...
            while player_1.total_score <= 100 and player_2.total_score <= 100:
                ...
                if player_2.is_holding is False:
                    ...
                    if player_2.total_score >= 100 or player_2.is_quitting is True:
                        break

            if player_1.total_score >= 100:
                print(f"Congratulations {player_1.name}, you beat {player_2.name}."
                      + f"Your longest streak was {player_1.longest_streak}")
                score_board.collect_score(player_1.name, player_1.total_score,
                                player_1.longest_streak, "highscore.txt")
            elif player_2.total_score >= 100:
                print(f"Congratulations {player_2.name}, you beat {player_1.name}."
                      + f"Your longest streak was {player_2.longest_streak}")
                score_board.collect_score(player_2.name, player_2.total_score,
                                 player_2.longest_streak, "highscore.txt")
        elif choice == "3":
